...
#        self.fpns = []
        ...
        self.fpn0 = nn.Sequential(
                upa_block(int(self.filters*30), 256, stride=1, l=1, k=1, same=True, bn=True),
                upa_block(256, 256, stride=1, l=1, k=3, same=True, bn=True),
#                CPA(int(self.filters*28), 256, same=True, bn=False),
                )
...
#        self.sem_con = upa_block(256, 256, stride=1, l=1, k=3, same=True)
        self.sem_head = upa_block(256, num_classes, stride=1, bn=True)
...
#        self.ins_con = upa_block(256, 256, stride=1, l=1, k=3, same=True)
        self.ins_head = upa_block(256, num_classes, stride=1, bn=True)
...
#        self.cls_head = upa_block(256 , int((num_classes+1)*ar), stride=1, same=True, bn=False)
        self.cls_head = nn.ModuleList([upa_block(256 , int((num_classes+1)*ar), stride=1, same=False, bn=True) for i in range(5)])
...
#        self.box_head = upa_block(256 , int(4*ar), stride=1, same=True, bn=False)
        self.box_head =  nn.ModuleList([upa_block(256 , int(4*ar), stride=1, same=False, bn=True) for i in range(5)])
...
#        self.coe_head = upa_block(256 , int(ar*num_classes), stride=1, w=1, same=True, bn=False)
        self.coe_head =  nn.ModuleList([upa_block(256 , int(ar*num_classes), stride=1, same=False, bn=True) for i in range(5)])
...
#        out_136 = F.interpolate(out_68s, size=(136, 136), mode='bilinear', align_corners=False)
...
#        ins_out = sem_out.permute(0, 2, 3, 1).contiguous()
        ...
        for i, outi in enumerate([out_68s, out_34s, out_17s, out_09s, out_05s]):
#            outi = self.fpn(outi)
...
#            cls_outi = self.cls_head_cpa(outi, cls_outi).permute(0, 2, 3, 1).reshape(outi.size(0), -1, 21)
            cls_outi = self.cls_head[i](outi).permute(0, 2, 3, 1).reshape(outi.size(0), -1, 21)
...
#            box_outi = self.box_head_cpa(outi, box_outi).permute(0, 2, 3, 1).reshape(outi.size(0), -1, 4)
            box_outi = self.box_head[i](outi).permute(0, 2, 3, 1).reshape(outi.size(0), -1, 4)
...
#            coe_outi = self.coe_head_cpa(outi, coe_outi).permute(0, 2, 3, 1).reshape(outi.size(0), -1, 20)
            coe_outi = self.coe_head[i](outi).permute(0, 2, 3, 1).reshape(outi.size(0), -1, 20)
