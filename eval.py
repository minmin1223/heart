...
from utils.output_utils import after_nms, nms, ma_nms
...
#        try:
        ...
        with timer.counter('nms'):
            ...
            # ids_p, class_p, box_p, coef_p, proto_p = ma_nms(class_p, box_p, coef_p, proto_p, net.anchors, cfg)
...
#            t_t, t_d, t_f, t_me = timer.get_times(['batch', 'data', 'forward', 'metric'])
            fps, t_fps = 1 / (t_d + t_f + t_nms + t_an), 1 / t_t
#            fps, t_fps = 1 / (t_d + t_f), 1 / t_t
...
#            print(f'\rTesting: {bar_str} {i + 1}/{ds}, fps: {fps:.2f} | total fps: {t_fps:.2f} | '
#                  f't_t: {t_t:.3f} | t_d: {t_d:.3f} | t_f: {t_f:.3f} | '
#                  f't_metric: {t_me:.3f}', end='')
    ...
    if cfg.coco_api:
        ...
        print(table)
#        losses_dict['losses_total']=losses_total
#        losses_dict['losses_c']=losses_c
#        losses_dict['losses_b']=losses_b
#        losses_dict['losses_s']=losses_s
#        losses_dict['losses_i']=losses_i
#        losses_dict['losses_i']=losses_m
#        losses_dict['itrs']=itrs
        
        return table, box_row, mask_row#, losses_dict
