...
def make_net(in_channels, conf, include_last_relu=True):
    ...
    """
    def make_layer(layer_cfg): # layer_cfg = conf[i] = cfg.mask_proto_net[i]
        nonlocal in_channels # in_channels (API) -> in_channels (make_layer) to return
        ...
        if isinstance(layer_cfg[0], str): # default:False
            ...
            num_channels = layer_cfg[0] # 256
            kernel_size = layer_cfg[1] # 3

            if kernel_size > 0: # True
        ...
        in_channels = num_channels if num_channels is not None else in_channels # 256
