...
def encode(matched, priors):
    #print("matched",matched.shape)  torch.Size([6175, 4])
    variances = [0.1, 0.2]
...
def ma_iou(cfg, bboxes1, bboxes2, gt_masks, eps=1e-6):
    ...
    Args:
        bboxes1 (Tensor): shape (m, 4) ==gt_box
        bboxes2 (Tensor): shape (n, 4) == decode_priors==predicted_anchors
    ...
    #print("_______bboxes1_______",bboxes1.shape,"_______bboxes2_______",bboxes2.shape) #[2,4] #[6175,4]
    lt = torch.max(bboxes1[:, None, :2], bboxes2[:, :2])  # [rows, cols, 2] #[2,6175,2]
    
    rb = torch.min(bboxes1[:, None, 2:], bboxes2[:, 2:])  # [rows, cols, 2]
    #print("bboxes1[:, None, :2]",bboxes1[:, None, :2],bboxes1[:, None, :2].shape)
    #print("bboxes2[:, :2]",bboxes2[:, :2].shape)
    #print("_______lt_______",lt.shape,"_______rb_______",rb.shape)  torch.Size([2, 6175, 2]), torch.Size([2, 6175, 2]

    wh = (rb - lt).clamp(min=0)  # [rows, cols, 2]
    #print("_______wh_______",wh.shape) torch.Size([2, 6175, 2])

    intersection = wh[:, :, 0] * wh[:, :, 1]
    #print("intersection",intersection.shape) [2, 6175])
    ...
    with torch.no_grad():
        ...
        #maller_ind = ious <= 0.
        #print("smaller_ind ",smaller_ind.shape)
        #print("ious",ious.shape)
        #print("ious[smaller_ind]",ious[smaller_ind[0]].shape)
        #print("ious[smaller_ind]",ious[smaller_ind].shape)
        ...
        for i in range(gt_number):
            ...
            #print("smaller_ind[i]",smaller_ind[i].shape)
            #overlap[i, smaller_ind[i]] = ious[i][smaller_ind[i]]
...
# 2. Label Assignment
# =============================================================================
def soft_simota(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes, box_p, class_p):
    ...
    c_pix_p, soft_label,reduction='none') * scale_factor.abs().pow(2.0)
...
"""
all_offsets[i], conf_gt[i], anchor_max_gt[i], anchor_max_i[i], mask_gt[i], csoft_simotanlass_gt[i] = 
match(self.cfg,box_gt,self.anchors,class_gt[i],mask_gt[i],self.num_level_bboxes,box_p[i],class_p[i])
"""
def dynamic_soft(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes, box_p, class_p):
    
    topk = 13
    iou_weight = 3.
    cls_weight = 1.
    soft_center_radius = 3.
    ...
    soft_center_prior = torch.pow(10, distance-soft_center_radius)
    ...
    cost_matrix = soft_center_prior + iou_weight * iou_cost + cls_weight * soft_cls_cost
...
def maiou_soft_simota(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes, box_p, class_p):
...
def real_simota(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes, box_p, class_p):
    #有maiou
    #print("_____it's using soft-simota_____")
    #topk = 13
    iou_weight = 3.
    ...
    gt_cy = (gt_bboxes[:, 1] + gt_bboxes[:, 3]) / 2.0
    gt_points = torch.stack((gt_cx, gt_cy), dim=1)
    #print("box_gt!!!",box_gt.shape)
    distance = torch.cdist(bboxes_priors_points, gt_points, p=2)  
    center_prior = torch.pow(10, distance)
    overlaps = box_iou(box_gt, decoded_priors, detail=False) 
    
    #overlaps, _, _ = maiou(cfg, box_gt*gt_masks.shape[1], decoded_priors*gt_masks.shape[1], gt_masks)

    #print("overlaps",overlaps.shape)
    #print("class_p",class_p.shape)
    ...
    overlaps = overlaps.transpose(0, 1)
    num_gt, num_bboxes = gt_bboxes.size(0), bboxes.size(0)
    #print("num_gt",num_gt,"num_bboxes" ,num_bboxes)
    ...
    gt_onehot_label = gt_onehot_label * is_in_box[:, :, None]
    #print("pix_p.shape",pix_p.shape)
    c_pix_p = pix_p.unsqueeze(1).repeat(1, num_gt, 1)
    #print("c_pix_p.shape",c_pix_p.shape)

    cls_cost = F.binary_cross_entropy_with_logits(
    c_pix_p, gt_onehot_label,reduction='none') 
    ...
    cls_cost = cls_cost.sum(-1).to(dtype=pix_p.dtype)
    """
    #print("cls_cost.shape",cls_cost.shape)
    #cost_matrix = soft_center_prior + iou_cost ** beta * cls_cost ** alpha # 原
    cost_matrix = center_prior + iou_weight * iou_cost + cls_cost
    #print("cost_matrix", cost_matrix.shape) #([6175,2])
    topk = torch.clamp(torch.round((c_overlaps).sum(0)), min=0.).int()
    #print("topk",topk) == 原作dynamic_ks
    """""
    _, candidate_idxs = (cost_matrix).topk(
                overlaps.shape[0], dim=0, largest=False)
    print("candidate_idxs",candidate_idxs.shape)
    c_overlaps = (c_overlaps)[candidate_idxs, torch.arange(num_gt).cuda()]  
    print("c_overlaps",c_overlaps.shape)
    print("topk",topk)
    for i, g in enumerate(topk):
        c_overlaps[g:, i] = 0.    #背景是0   
    """
    matching_matrix = torch.zeros_like(cost_matrix) # 應該是跟overlaps_inf很像
    #n_candidate_k = min(200,overlaps.size(0))
    ordered_cost_matrix, candidate_idxs = (cost_matrix).topk(
        overlaps.shape[0], dim=0, largest=False)
    topk = torch.clamp(torch.round((c_overlaps).sum(0)), min=0.).int()

    for gt_idx in range(num_gt):
        #print("cost_matrix",cost_matrix.shape)
        #print("cost_matrix",cost_matrix[0].shape)
        #print("cost_matrix[:,gt_idx]",cost_matrix[:,gt_idx].shape) # torch.Size([6175])
        #print("topk",topk.shape)
        k = int(topk[gt_idx])
        #print("topk[gt_idx]",k,type(k))
        _, pos_idx = ordered_cost_matrix[:,gt_idx].topk(k=k,largest=False)
        #print("matching_matrix",matching_matrix.shape)
        #print("matching_matrix[gt_idx,pos_idx]",matching_matrix[pos_idx,gt_idx].shape)
        matching_matrix[pos_idx,gt_idx]=1

    anchor_matching_gt = matching_matrix.sum(1)
    #print("anchor_matching_gt",anchor_matching_gt.shape)

    if anchor_matching_gt.max() >1:
        multiple_match_mask = anchor_matching_gt > 1
        #print("multiple_match_mask",multiple_match_mask.shape)
        #print("cost_matrix[multiple_match_mask,:]",cost_matrix[multiple_match_mask,:].shape)
        _ , cost_argmin = ordered_cost_matrix[multiple_match_mask,:].min(dim=1)
        #print("cost_argmin",cost_argmin)
        #print("multiple_match_mask",multiple_match_mask)
        matching_matrix[multiple_match_mask, :] *= 0
        matching_matrix[multiple_match_mask,cost_argmin] = 1
        #print("multiple_match_mask",multiple_match_mask.shape)
    """
        現在用matching_matrix取代原本的c_overlaps
       cost matrix中如果兩個gt的(兩列)用同一個anchor 在candidate_idxs會有一樣編號
       candidate_idxs[topk[0],gt1] 中 與 candidate_idxs[topk[1],gt2] 中 有重複的數字，便是重複的編號
       到cost_matrix[candidate_idxs[topk[0]],gt1]  與 cost_matrix[candidate_idxs[topk[1]],gt2] 中比較，
       把較大的cost的刪除 或許用c_overlaps對應編號改0
    """
    #print("matching_matrix",matching_matrix.shape)  
    #print("c_overlaps",c_overlaps.shape)
    is_pos = (matching_matrix > 0.)
    #print("is_pos",is_pos.shape) #[6175,2]

    for gt_idx in range(num_gt):
        candidate_idxs[:, gt_idx] += gt_idx * num_bboxes # 6175

    overlaps_inf = torch.full_like(overlaps, -float('inf')).t().contiguous().view(-1)   
    #print("overlaps",overlaps.shape)
    #print("overlaps_inf_1",overlaps_inf.shape)
    #print("cost_matrix.view(-1)",cost_matrix.view(-1).shape)
    #print("is_pos.view(-1)",is_pos.contiguous().view(-1).shape)
    index = candidate_idxs.view(-1)[is_pos.reshape(-1)] #在candidate_idxs中iou coverlaps大於0的正樣本
    #print("index",index.shape)
    overlaps_inf[index] = overlaps.t().contiguous().view(-1)[index] # 在iou overlaps中的coverlaps大於0的正樣本 應該可以改成用costmatrix
    # anchor_max_gt: the corresponding max IoU gt box for each anchor
    overlaps_inf = overlaps_inf.reshape(num_gt, -1).t()
    #print("overlaps_inf_2",overlaps_inf.shape)
    #print("overlaps_inf_2",overlaps_inf)
    # 論文是說有兩個anchor共用到一個gt的話，用cost決定，但這裡用IOU。cost matrix只在選擇正樣本時用到

    #assigned_gt_inds = overlaps.new_full((num_bboxes, ),0,dtype=torch.long)

    max_overlaps, argmax_overlaps = overlaps_inf.max(dim=1) # max_overlaps是對大的iou, argmax_overlaps是索引
    assigned_gt_inds[max_overlaps != -float('inf')] = argmax_overlaps[max_overlaps != -float('inf')] + 1

    #print("max_overlaps",max_overlaps.shape,"argmax_overlaps",argmax_overlaps.shape)
    #print("argmax_overlaps",argmax_overlaps)
    #print("assigned_gt_inds",assigned_gt_inds.shape)
    #print("overlaps_inf_2",overlaps_inf.shape)
    #print("overlaps_inf_2",overlaps_inf)
    ...
    if gt_labels is not None:
        ...
        if pos_inds.numel() > 0:
            assigned_labels[pos_inds] = gt_labels[
                assigned_gt_inds[pos_inds] - 1] + 1 # * m
            max_overlaps[pos_inds]
            #print("pos_inds",pos_inds.shape) # positive樣本數（topk數量）
   
            
            
    overlaps, conf, anchor_max_i, is_pos = overlaps, assigned_labels, argmax_overlaps, overlaps_inf
    #print("overlaps_inf",overlaps_inf.shape)
    #print("box_gt",box_gt.shape)
    anchor_max_gt = box_gt[anchor_max_i] 
    #print("anchor_max_i",anchor_max_i.shape)
    offsets = encode(anchor_max_gt, anchors) 
    #print(offsets)
    #print("c_overlaps",c_overlaps.shape) #([6175,2])
    # maious(Tensor): shape (m, n) (gt,anchor) # overlaps(Tensor): shape (n, m) (anchor,gt)
    return offsets, conf, anchor_max_gt, anchor_max_i, gt_masks, class_gt



def atss(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes):
    # from soft-simota    decoded_box = torch.cat((anchors[:, :2] + box_p[:, :2] * 0.1 * anchors[:, 2:],
                          #anchors[:, 2:] * torch.exp(box_p[:, 2:] * 0.2)), 1)
    # atss(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes)  
    # anchors為[x,y,w,h]
    # decoded_priors是預測出來的anchors的上下左右座標
    decoded_priors = torch.cat((anchors[:, :2] - anchors[:, 2:] / 2, anchors[:, :2] + anchors[:, 2:] / 2), 1)

    bboxes = decoded_priors
    gt_bboxes = box_gt
    gt_labels = class_gt
    # ma_iou here #def ma_iou(cfg, bboxes1, bboxes2, gt_masks, eps=1e-6):
    #overlaps, _, _ = ma_iou(cfg, box_gt*gt_masks.shape[1], decoded_priors*gt_masks.shape[1], gt_masks)
    overlaps = box_iou(box_gt, decoded_priors, detail=False) 
...
def match(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes, box_p, class_p):
    ...
    # 由 yolact.py 一個一個batch處理
    if cfg.label_assignment == 'maiou':
        return atss(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes)    
    if cfg.label_assignment == 'soft_simota':
        return soft_simota(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes, box_p, class_p)    
    if cfg.label_assignment == 'real_simota':
        return real_simota(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes, box_p, class_p)    
    if cfg.label_assignment == 'maiou_soft_simota':
        return maiou_soft_simota(cfg, box_gt, anchors, class_gt, gt_masks, num_level_bboxes, box_p, class_p)     